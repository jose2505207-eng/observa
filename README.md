...
- **QNN/NPU acceleration** — the full pipeline is built: a real HTP-lowered `QnnBackend` `.pte` (raw-head export), the v79 runtime libs packaged in `jniLibs`, and runtime backend selection that prefers QNN. On the **retail S25 Ultra** the `.pte` loads but the Hexagon DSP refuses the unsigned HTP skel (`error 4000`) — confirmed an **OS-level retail lock** (Google's official LiteRT Qualcomm delegate fails identically; the camera uses the DSP fine), needing a signed/engineering build. OBSERVA **falls back to the XNNPACK CPU delegate (32 ms, under target) and reports it honestly** — `LOADED_QNN` is set only when a QNN warm-up `forward` truly succeeds. See [`docs/implementation/MODEL_RUNTIME.md`](docs/implementation/MODEL_RUNTIME.md).
