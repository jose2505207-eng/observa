...
- **Visible mode UI**: high-contrast, TalkBack-labeled buttons (no icon-only) — **Awareness · Navigate · Translate · Voice Commands · Read Signs · Repeat Alert** — plus a **Navigation** card (live bearing guidance + offline map-pack status), a **Translation** card (offline readiness), and a collapsible **Debug Status** card (backend, QNN stage/error, map/language packs, GPS, compass, OCR, voice, INTERNET, build provenance). Navigation works with compass bearing even with no map pack; a pack only adds the rendered/route layer. See [`docs/implementation/OFFLINE_MAPS.md`](docs/implementation/OFFLINE_MAPS.md).
...
- **QNN/NPU acceleration** — the full pipeline is built: a real HTP-lowered `QnnBackend` `.pte` (raw-head export), the v79 runtime libs packaged in `jniLibs`, and runtime backend selection that prefers QNN. **Re-validated 2026-06-29 from scratch** (rebuilt the QNN host adaptor, re-exported the HTP model — both pass; `.pte` loads on device): on the **retail S25 Ultra** only the on-device HTP skel load is refused (`error 4000`) — confirmed an **OS-level retail lock, not a laptop-build artifact** (Google's official LiteRT Qualcomm delegate fails identically; in the same logcat the signed camera opens an unsigned PD on that DSP), needing a signed/engineering build. OBSERVA **falls back to the XNNPACK CPU delegate (~22–32 ms, under target) and reports it honestly** — `LOADED_QNN` is set only when a QNN warm-up `forward` truly succeeds. See [`docs/implementation/MODEL_RUNTIME.md`](docs/implementation/MODEL_RUNTIME.md).
